...
**The result:** TDD actually happens. Architecture decisions get challenged before code is written. Security review is automatic. And when you start a new session, agent-notes in every file mean Antigravity doesn't start from zero.
...
3. **Agent-notes** — structured metadata at the top of every file so Antigravity remembers context between sessions
4. **27 skills workflows** covering the full development lifecycle (`/kickoff`, `/tdd`, `/code-review`, etc.)
...
Run one of the scaffold skills to set up your project structure:

| skills | What it sets up |
|---------|----------------|
...
You talk to Antigravity normally. The template's AGENTS.md file teaches Antigravity when to invoke each agent based on what phase your work is in.
...
| **Agent-notes** | Metadata at the top of every file (`ctx`, `deps`, `state`, `last`) so Antigravity can quickly understand any file without reading the whole thing. |
...
| `.agents/skills/*/SKILL.md` | 30 workflow skills/skills |
| `docs/team-directives.md` | Low-ceremony project conventions |

Directories like `docs/code-reviews/`, `docs/plans/`, `docs/sprints/`, `docs/tracking/`, `docs/sbom/`, `docs/security/`, and `docs/runbooks/` are created automatically by skills when first needed — they don't exist in the template.

## Antigravity Native Features

Unlike standard text-based editors, Antigravity has native capabilities that the VTeam framework leverages:
- **Artifacts:** Code reviews, plans, and ADRs are generated as interactive UI Artifacts using Markdown elements like Carousels, GitHub Alerts, and Mermaid Diagrams.
- **Knowledge Items:** Antigravity learns across sessions automatically. You can explicitly trigger this using the `/learn` slash skills.
- **Planning Mode:** Antigravity has a built-in planning mode with dedicated artifacts (`task.md`, `implementation_plan.md`, `walkthrough.md`) which the `/vteam-plan` skill utilizes.

## skills Reference

All skills are invoked as `/<name>` in Antigravity (e.g., `/kickoff`, `/tdd`).

### Native skills
| skills | Description |
|---------|-------------|
| `goal` | Set a long-running or autonomous goal for the agent |
| `schedule` | Run skills in the background on a schedule or timer |
| `grill-me` | Interactive interview to align on design decisions |
| `learn` | Store current knowledge or corrections persistently for future sessions |
...
| `vteam-plan` | Create an implementation plan for a feature using native artifacts |
| `tdd` | TDD workflow: Tara writes failing tests, Sato implements |
| `code-review` | Three-lens code review (simplicity, tests, security) using UI Artifacts |
| `vteam-review` | Guided human review/walkthrough session |
| `sprint-boundary` | Sprint retro, backlog sweep, next sprint setup |
| `handoff` | Save session state for the next session to resume |
| `vteam-resume` | Pick up from a previous session's handoff |
| `vteam-retro` | Kaizen retrospective with GitHub issues for findings |
| `vteam-preview`| Orchestrator agent that invokes multiple agents (Dani, Vik, Pierrot) to preview an MVP |
...
| `dev-server` | Detect and launch a background dev server |
| `deploy` | Dynamically generate MVP deployment configuration |
...
Create `.agents/skills/<name>/SKILL.md` with an agent-notes HTML comment as the first line. skills are auto-discovered by Antigravity.
...
- **Solo / small project:** Collapse further. Sato absorbs Ines's infra work. The code-reviewer skills covers all review. Pat handles all planning.
