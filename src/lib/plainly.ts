...
class PlainlyClient {
    ...
    async waitForRender(renderId: string, maxWait = 300000): Promise<PlainlyRender> {
        ...
        while (Date.now() - start < maxWait) {
            ...
            if (render.state === "FAILED") {
                // Pass through Plainly's error detail for debuggability
                const detail = (render as any).errorMessage || (render as any).error || "No details provided";
                throw new Error(`Plainly render ${renderId} failed: ${detail}`);
            }
    ...
    /**
     * Delete a project (cleanup) with verification.
     * After DELETE, waits briefly and confirms the project is gone (404).
     * Retries once if it is still present.
     */
    async deleteProject(projectId: string): Promise<void> {
        ...
        if (!res.ok && res.status !== 404) {
            console.warn(`deleteProject ${projectId}: initial DELETE returned ${res.status}`);
        }

        // Wait 2 seconds for deletion to propagate
        await new Promise(r => setTimeout(r, 2000));

        // Verify deletion by attempting a GET — expect 404
        const verifyRes = await fetch(`${PLAINLY_BASE_URL}/projects/${projectId}`, {
            headers: this.headers,
        });
        ...
        // Still present — retry delete once
        console.warn(`deleteProject ${projectId}: project still exists after delete, retrying...`);
