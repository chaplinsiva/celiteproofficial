...
export async function processRenderJob(renderJobId: string, isSample: boolean = false) {
    ...
    try {
        ...
        // ── 12. Deduct render credit AFTER confirmed success ────────────────────
        if (!sampleMode && job.subscription_id) {
            try {
                // Safely read current count then increment by 1
                ...
                console.error("Failed to increment renders_used (non-critical):", creditErr);
            }
        }
        ...
        if (sampleMode) {
            try {
                ...
                console.log(`✅ Free preview logged for user ${job.user_id} after confirmed success.`);
            } catch (logError) {
        ...
        // Cleanup Plainly project if it was created
        if (plainlyProjectId) {
            try {
                await plainlyClient.deleteProject(plainlyProjectId);
