...
export function verifyPaymentSignature(
    ...
    try {
        ...
        // If buffers have different lengths (malformed signature), they don't match
        return false;
    }
}
...
/**
 * Get the global single pay amount (in paise)
 */
export async function getSinglePayAmount(): Promise<number> {
    const { data, error } = await supabaseAdmin
        .from('razorpay_config')
    ...
    if (error || !data) {
        return 69900; // Fallback to ₹699
    }

    return data.single_pay_amount || 69900;
}
