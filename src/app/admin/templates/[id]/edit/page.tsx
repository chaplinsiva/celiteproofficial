...
import {
    ...
    Type, Plus, X, Loader2, Save, Search, Sparkles, Eye, EyeOff
} from "lucide-react";
...
export default function EditTemplatePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [creditCost, setCreditCost] = useState<number>(20);
    const [previewUrl, setPreviewUrl] = useState("");
    const [thumbnailUrl, setThumbnailUrl] = useState("");
    const [sourceUrl, setSourceUrl] = useState("");
    const [oneTimePrice, setOneTimePrice] = useState<number>(10000);
    ...
    const fetchTemplate = async () => {
        try {
            ...
            setCreditCost(t.credit_cost ?? 20);
            setOneTimePrice(t.one_time_price ?? 10000);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const filesMeta: Record<string, { name: string; type: string }> = {};
            if (previewFile) filesMeta.preview = { name: previewFile.name, type: previewFile.type };
            if (thumbnailFile) filesMeta.thumbnail = { name: thumbnailFile.name, type: thumbnailFile.type };
            if (sourceFile) filesMeta.source = { name: sourceFile.name, type: sourceFile.type };

            const refUploadKeys = Object.keys(referenceImageFiles).filter(k => referenceImageFiles[k]);
            refUploadKeys.forEach(key => {
                const file = referenceImageFiles[key];
                if (file) {
                    filesMeta[`reference_${key}`] = { name: file.name, type: file.type };
                }
            });

            if (Object.keys(filesMeta).length > 0) {
                setUploadProgress("Getting secure upload links...");
                ...
                const presignRes = await fetch("/api/admin/upload", {
                    ...
                    body: JSON.stringify({ slug, files: filesMeta }),
                });
                ...
                setUploadProgress("Uploading files directly to cloud...");
                const uploadPromises = [];

                const uploadFile = async (key: string, file: File) => {
                    const data = presignData.urls[key];
                    if (!data) return null;
                ...
                if (previewFile) {
                    uploadPromises.push(uploadFile("preview", previewFile).then(url => { if (url) newPreviewUrl = url; }));
                }
                if (thumbnailFile) {
                    uploadPromises.push(uploadFile("thumbnail", thumbnailFile).then(url => { if (url) newThumbnailUrl = url; }));
                }
                if (sourceFile) {
                    uploadPromises.push(uploadFile("source", sourceFile).then(url => { if (url) newSourceUrl = url; }));
                }

                for (const key of refUploadKeys) {
                    const file = referenceImageFiles[key];
                    if (file) {
                        uploadPromises.push(uploadFile(`reference_${key}`, file).then(url => {
                            if (url) {
                                const idx = imagePlaceholders.findIndex(p => p.key === key);
                                if (idx !== -1) {
                                    imagePlaceholders[idx].referenceImageUrl = url;
                                }
                            }
                        }));
                    }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-2 block flex items-center gap-2">
                                        <Sparkles className="w-3 h-3 text-emerald-400" /> Credit Cost
                                    </label>
                                    <input
                                        type="number"
                                        min="0"
                                        ...
                                        className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-1 focus:ring-emerald-500/50 focus:outline-none"
                                    />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-2 block flex items-center gap-2">
                                        <Sparkles className="w-3 h-3 text-amber-400" /> One-Time Price (paise)
                                    </label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={oneTimePrice}
                                        onChange={(e) => setOneTimePrice(parseInt(e.target.value) || 0)}
                                        className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-1 focus:ring-amber-500/50 focus:outline-none"
                                    />
                                    <span className="text-xs text-gray-500 mt-1 block">₹{(oneTimePrice / 100).toFixed(0)} displayed to users</span>
                                </div>
                                <div className="md:col-span-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Premium Template</label>
                                    <div className="flex items-center gap-3">
                                        <button
                                            type="button"
                                            onClick={() => setIsPremium(!isPremium)}
                                            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${
                                                isPremium ? 'bg-amber-500' : 'bg-white/10'
                                            }`}
                                        >
                                        ...
                                        <span className="text-xs text-gray-500">
                                            {isPremium ? 'Subscription only — one-time purchase disabled' : 'One-time purchase available'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    >
                                        <div className="flex-1 grid grid-cols-5 gap-3">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => updateImagePlaceholder(index, "previewTimestamp", e.target.value ? parseFloat(e.target.value) : "")}
                                                className="bg-white/[0.03] border border-white/10 rounded-lg py-2 px-3 text-xs text-white focus:outline-none"
                                                placeholder="Sec"
                                                step="0.1"
                                            />
                                    ...
                                    >
                                        <div className="flex-1 grid grid-cols-4 gap-3">
                                            ...
                                            <input
                                                type="number"
                                                value={placeholder.previewTimestamp ?? ""}
                                                onChange={(e) => updateTextPlaceholder(index, "previewTimestamp", e.target.value ? parseFloat(e.target.value) : "")}
                                                className="bg-white/[0.03] border border-white/10 rounded-lg py-2 px-3 text-xs text-white focus:outline-none"
                                                placeholder="Sec"
                                                step="0.1"
                                            />
