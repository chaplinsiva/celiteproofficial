...
export default function NewTemplatePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const hasFiles = previewFile || thumbnailFile || sourceFile;
            ...
            if (hasFiles) {
                ...
                const handleDirectUpload = async (key: string, file: File) => {
                    ...
                    uploadedUrls[`${key}_url`] = data.publicUrl;
                };

                if (previewFile) uploadPromises.push(handleDirectUpload("preview", previewFile));
                if (thumbnailFile) uploadPromises.push(handleDirectUpload("thumbnail", thumbnailFile));
                if (sourceFile) uploadPromises.push(handleDirectUpload("source", sourceFile));
            ...
            const templateRes = await fetch("/api/admin/templates", {
                ...
                body: JSON.stringify({
                    ...
                    preview_url: uploadedUrls.preview_url || null,
                    thumbnail_url: uploadedUrls.thumbnail_url || null,
                    source_url: uploadedUrls.source_url || null,
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-1 focus:ring-emerald-500/50 focus:outline-none transition-all"
                                    />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-2 block flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-1 focus:ring-amber-500/50 focus:outline-none transition-all"
                                    />
